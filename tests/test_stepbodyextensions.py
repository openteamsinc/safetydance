...
from ast import NodeTransformer, Name, Index, Call, Str, Num, Expr, Eq, Subscript, Load, Store, Assert, Compare, dump
           ...
           and node.targets[0].value.id == 'context'):
            ...
            called_func = Call\
               (func=Name(id='print', ctx=Load()), \
                ...
                ''')], \
                keywords=[])
            newnode = Expr(targets=[Subscript(value=Name(id='context', ctx=Load()),\
                                              slice=Index(value=Name(id='x', ctx=Load())), ctx=Store())],\
                                              value=called_func)
            return [node, newnode]
